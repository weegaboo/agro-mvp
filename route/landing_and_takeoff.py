...
from geo.crs import CRSContext, to_wgs_geom
...
def build_wpl_from_local_route(
    runway_m: LineString,
    route_points_m: List[Point],                 # CCA..FAF (последняя точка = FAF)
    ...
    *,
    ...
    repeat_faf_waypoint: bool = False,           # <- можно убрать «повтор FAF»
    dedupe_eps_m: float = 0.5                    # <- дедуп: точки ближе этой дистанции считаем дублями
) -> str:
    (x0, y0), (x1, y1) = map(tuple, runway_m.coords[:2])
    vx, vy = x1 - x0, y1 - y0
    ...
    # ---- helper: дедуп подряд идущих точек в локальных метрах ----
    def _dedupe(seq_pts: List[Point], eps=dedupe_eps_m) -> List[Point]:
        out = []
        last = None
        for p in seq_pts:
            if last is None or p.distance(last) > eps:
                out.append(p)
                last = p
        return out
    ...
    lines.append(f"{seq} 1 {FRAME} 22 0 0 0 0 {lat0:.7f} {lon0:.7f} {float(takeoff_cfg['takeoff_alt_agl']):.2f} {AUTO}"); seq += 1
    ...
    if include_midpoint_on_rw:
        ...
        lines.append(f"{seq} 0 {FRAME} 16 0 0 0 0 {mid_wgs.y:.7f} {mid_wgs.x:.7f} {cruise_alt_agl:.2f} {AUTO}"); seq += 1
    ...
    lines.append(f"{seq} 0 {FRAME} 16 0 0 0 0 {wp_after_tko_wgs.y:.7f} {wp_after_tko_wgs.x:.7f} {cruise_alt_agl:.2f} {AUTO}"); seq += 1

    # 3) ТЕПЕРЬ — DO_CHANGE_SPEED (чтобы не шла первой в MP)
    lines.append(f"{seq} 0 {FRAME} 178 0 {float(takeoff_cfg['speed_ms']):.3f} 0 0 0 0 0 {AUTO}"); seq += 1

    # 4) ваш маршрут на крейсерской высоте
    if not route_points_m:
        raise ValueError("route_points_m is empty: требуется хотя бы одна точка (включая FAF).")
    route_wgs: List[Point] = [to_wgs_geom(pt, ctx) for pt in route_points_m]
    for pt_wgs in route_wgs:
        lines.append(f"{seq} 0 {FRAME} 16 0 0 0 0 {pt_wgs.y:.7f} {pt_wgs.x:.7f} {cruise_alt_agl:.2f} {AUTO}"); seq += 1

    # 5) DO_LAND_START @ FAF (последняя точка маршрута = FAF)
    faf_alt = float(landing_cfg.get("faf_alt_agl", cruise_alt_agl))
    faf_wgs: Point = route_wgs[-1]
    lines.append(f"{seq} 0 {FRAME} 189 0 0 0 0 {faf_wgs.y:.7f} {faf_wgs.x:.7f} {faf_alt:.2f} {AUTO}"); seq += 1

    # 6) (опц.) повтор FAF как WAYPOINT
    if repeat_faf_waypoint:
        lines.append(f"{seq} 0 {FRAME} 16 0 0 0 0 {faf_wgs.y:.7f} {faf_wgs.x:.7f} {faf_alt:.2f} {AUTO}"); seq += 1
